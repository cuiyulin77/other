...
class HuanqiuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page_num = re.match(r".*?(\d+)\.html$",next_url).group(1)
        if int(next_page_num) < 10:
    ...
    def get_content(self,response):
        ...
        item['media'] = '环球网'
        item['come_from'] = response.xpath("//span[@class='la_t_b']/a/text()").extract_first()
        ...
        if not item['title']:
            item['title'] = response.xpath("//h1/strong/text()").extract_first()
            html = response.body.decode()
            html =  html.replace('\n','').replace(' ','')
            html_re = re.search(r'\"img\":(\[.*?\])', html, re.S)
            if html_re:
                html_json = json.loads(html_re.group(1))
                item['content'] = html_json[0]['title'].replace(u'\u3000', u' ').replace(u'\xa0', u' ')
        try:
