...
class CnmoSpider(scrapy.Spider):
    name = 'cnmo'
    allowed_domains = ['cnmo.com/news','cnmo.com','cnmo.com/reviews','cnmo.com/news/date']
    ...
    def parse(self, response):
        for url in self.url_list:
            # print('1'*100)
            yield scrapy.Request(url,callback=self.parse_detail,dont_filter=True)   # dont_filter=True

    def parse_detail(self,response):
        # print('&'*100, 222222)
        url_list = response.xpath("//div[@class='Newcon-title clearfix']/a/@href").extract()
        for url in url_list:
            url_real = 'http:'+url
            print(url_real)
            yield scrapy.Request(url_real,callback=self.get_content,)   # dont_filter=True
