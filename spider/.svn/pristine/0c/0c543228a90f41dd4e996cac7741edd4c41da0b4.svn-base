...
class XinhuaSpider(scrapy.Spider):
    ...
    def parse_detail(self,response):
        ...
        for content in content_list:
            ...
            item['media'] = '新华网'     #content['SourceName']
            yield scrapy.Request(item['url'],callback=self.get_content,meta={'item':deepcopy(item)},dont_filter=True)

    def get_content(self,response):
        item = response.meta['item']
        item['content'] = response.xpath("//div[@id='p-detail']//p//text()").extract()
        if not item['content']:
            item['content'] = response.xpath("//div[@id='article']//p//text()").extract()
            if not item['content']:
                item['content'] = response.xpath("//span[@id='content']/p[2]/text()").extract()
        if item['content']:
            item['content'] = ''.join(item["content"]).replace(u'\u3000', u' ').replace(u'\xa0', u' ')
            item['come_from'] = response.xpath("//em[@id='source']/text()").extract_first().replace('\n','').replace('\r','')
