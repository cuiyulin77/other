...
class QqnewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for url in self.url_list:
            yield scrapy.Request(url, callback=self.parsepage,dont_filter=True)  # dont_filter=True)
    ...
    def parsebody(self, response):
        ...
        if not item['content'] == '':
            ...
            yield scrapy.Request(com_url,callback=self.get_comm_num, meta={'item':item})    # dont_filter=True,
