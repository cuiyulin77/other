...
def get_ip():
    ...
    html_get = requests.get(
        'http://webapi.http.zhimacangku.com/getip?num=5&type=2&pro=&city=0&yys=0&port=1&pack=21479&ts=1&ys=0&cs=0&lb=1&sb=0&pb=45&mr=1&regions=')
...
def get_ip1():
    # 一次提取5个
    html_get = requests.get(
        'http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=1&pack=21479&ts=1&ys=0&cs=0&lb=1&sb=0&pb=45&mr=1&regions=')
    get_json = json.loads(html_get.content.decode())
    # print(type(get_json['success']))
    if get_json['success']:
        ip_list = get_json['data']
